...
%operations, plus a random array of [0 1..] that is of length 'list length'
%and indicates whether an operation should be true (1) or not (0). The
%function returns an array of operations for that trial whose truth value
%matches the input array of [0 1...]. The function assumes that true
%operations are in column 1 of the cell array, and false operations in
%column 2. The rows are indexed by global variables mfulptr and mlessptr
%which are advanced after each operation has been assigned.

function ss4trial = getops (allsentences, meanings)
