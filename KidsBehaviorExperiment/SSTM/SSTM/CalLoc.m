...
function Loc=CalLoc(Size,Gap,DotIndex,AdjH,AdjV,p)
   
   if p==1%Check if widescreen is used
      Loc(1,1)=((151.5*AdjH)+(DotIndex(1,1)-1)*50+Gap)*AdjH;
      Loc(1,2)=(51.5+(DotIndex(1,2)-1)*50+Gap)*AdjV;
      Loc(1,3)=((151.5*AdjH)+(DotIndex(1,1)-1)*50+Gap+Size)*AdjH;
      Loc(1,4)=(51.5+(DotIndex(1,2)-1)*50+Gap+Size)*AdjV;
   else
      Loc(1,1)=(151.5+(DotIndex(1,1)-1)*50+Gap)*AdjH;
      Loc(1,2)=(51.5+(DotIndex(1,2)-1)*50+Gap)*AdjV;
      Loc(1,3)=(151.5+(DotIndex(1,1)-1)*50+Gap+Size)*AdjH;
      Loc(1,4)=(51.5+(DotIndex(1,2)-1)*50+Gap+Size)*AdjV;
   end
end