function [ firstMax, secondMax, thirdMax, fourthMax, fifthMax, sixthMax, seventhMax violations ] = twoItemGARPChoiceOmit(violation, violationbychoice, set);
...
violationsRev6.twoItemGARP = violationsRev5.twoItemGARP;
for t = 1:length(violationsRev6.twoItemGARP)
    if violationsRev6.twoItemGARP(indexMax6,t) ~= 0
        violationsRev6.twoItemGARP(indexMax6,t) = violationsRev6.twoItemGARP(indexMax6,t) - 1;
    end
end
for t = 1:length(violationsRev6.twoItemGARP)
    if violationsRev6.twoItemGARP(t,indexMax6) ~= 0
        violationsRev6.twoItemGARP(t,indexMax6) = violationsRev6.twoItemGARP(t,indexMax6) - 1;
    end
end
violationbychoiceRev6 = zeros(length(set),1);
for t = 1:length(set)
    violationbychoiceRev6(t,1) = nnz(violationsRev6.twoItemGARP(t,:));
end
